...
static int alea_ponderata(const double *pondera, int n) {
    double summa = 0;
    for (int i = 0; i < n; i++) summa += pondera[i];
    double r = alea_unitas() * summa;
    double acc = 0;
    for (int i = 0; i < n; i++) { acc += pondera[i]; if (r < acc) return i; }
    return n - 1;
}
...
typedef struct { const char *textus; const char *cutes; } ColorComae;
...
typedef struct { const char *textus; char sexus; char qualitas; } Lineamentum;
...
typedef struct { const char *textus; char categoria; } ColorCutis;
...
typedef struct { const char *textus; char sexus; } Vestimentum;
...
static const char *elige_vestem(const Vestimentum *series, int n, char sexus_c) {
    int indices[256]; int k = 0;
    for (int i = 0; i < n && k < 256; i++) {
        if (series[i].sexus == 'u' || series[i].sexus == sexus_c) indices[k++] = i;
    }
...
static const char *elige_lineamentum(const Lineamentum *series, int n, char sexus_c, int pulchritudo) {
    const char *permissa;
    switch (pulchritudo) {
        case 0: case 1: permissa = "un"; break;
        case 2:         permissa = "n";  break;
        default:        permissa = "np"; break;
    }
    int indices[256], k = 0;
    for (int i = 0; i < n && k < 256; i++) {
...
static const char *elige_colorem_comae_iuvenem(char cutis_categoria) {
    ...
    for (int i = 0; i < n && k < 64; i++) {
        if (strchr(colores_comae_iuvenum[i].cutes, cutis_categoria)) indices[k++] = i;
    }
...
char *persona_generare(uint64_t semen,
    ...
    if (arg_aetas) {
        ...
        if      (n <= 12) aetas = AETAS_PUER;
        else if (n <= 17) aetas = AETAS_ADULESCENS;
        else if (n <= 27) aetas = AETAS_IUVENIS;
        else if (n <= 39) aetas = AETAS_ADULTUS;
        else if (n <= 59) aetas = AETAS_MATURUS;
        else              aetas = AETAS_SENEX;
    } else {
        aetas = (Aetas)alea_ponderata(pondera_aetatum, 6);
    }
    int aetas_anni;
    switch (aetas) {
        case AETAS_PUER:       aetas_anni = 5  + alea_ambitus(8);  break;
        case AETAS_ADULESCENS: aetas_anni = 13 + alea_ambitus(5);  break;
        case AETAS_IUVENIS:    aetas_anni = 18 + alea_ambitus(10); break;
        case AETAS_ADULTUS:    aetas_anni = 28 + alea_ambitus(12); break;
        case AETAS_MATURUS:    aetas_anni = 40 + alea_ambitus(20); break;
        default:               aetas_anni = 60 + alea_ambitus(30); break;
    }

    Sexus sexus;
    if (arg_sexus) {
        if      (!strcmp(arg_sexus, "m") || !strcmp(arg_sexus, "mas"))    sexus = SEXUS_MAS;
        else if (!strcmp(arg_sexus, "f") || !strcmp(arg_sexus, "femina")) sexus = SEXUS_FEMINA;
        else {
    ...
    if (arg_cutis) {
        if      (!strcmp(arg_cutis, "clara"))   cutis_categoria = 'L';
        else if (!strcmp(arg_cutis, "media"))   cutis_categoria = 'M';
        else if (!strcmp(arg_cutis, "obscura")) cutis_categoria = 'D';
        else {
    ...
    for (int i = 0; i < numerus_cutium; i++)
        if (colores_cutis[i].categoria == cutis_categoria) congruentes[numerus_congruentium++] = i;
    ...
    if (aetas >= AETAS_SENEX) {
        ...
        color_comae = elige_unum(colores_comae_canescentes, sizeof colores_comae_canescentes / sizeof *colores_comae_canescentes);
    ...
    if (sexus == SEXUS_MAS) {
        if (longitudo_comae == 0) capillatura = elige_unum(capillaturae_virorum_breves, sizeof capillaturae_virorum_breves / sizeof *capillaturae_virorum_breves);
        else if (longitudo_comae == 1) capillatura = elige_unum(capillaturae_virorum_mediae, sizeof capillaturae_virorum_mediae / sizeof *capillaturae_virorum_mediae);
        else capillatura = elige_unum(capillaturae_virorum_longae, sizeof capillaturae_virorum_longae / sizeof *capillaturae_virorum_longae);
    } else {
        if (longitudo_comae == 0) capillatura = elige_unum(capillaturae_feminarum_breves, sizeof capillaturae_feminarum_breves / sizeof *capillaturae_feminarum_breves);
        else if (longitudo_comae == 1) capillatura = elige_unum(capillaturae_feminarum_mediae, sizeof capillaturae_feminarum_mediae / sizeof *capillaturae_feminarum_mediae);
        else capillatura = elige_unum(capillaturae_feminarum_longae, sizeof capillaturae_feminarum_longae / sizeof *capillaturae_feminarum_longae);
    }

    /* Calvities viris senioribus possibilis. */
    int calvus = 0;
    if (sexus == SEXUS_MAS && aetas >= AETAS_MATURUS && alea_fors(aetas >= AETAS_SENEX ? 0.5 : 0.25)) calvus = 1;
    ...
    if (sexus == SEXUS_MAS && aetas >= AETAS_IUVENIS) {
        ...
        if (alea_fors(p)) barba = elige_unum(barbae, sizeof barbae / sizeof *barbae);
    }
    ...
    const char *oculi = elige_lineamentum(colores_oculorum, sizeof colores_oculorum / sizeof *colores_oculorum, sexus_littera, pulchritudo);
    const char *vultus = elige_lineamentum(formae_vultus, sizeof formae_vultus / sizeof *formae_vultus, sexus_littera, pulchritudo);
    const char *nasus = elige_lineamentum(formae_nasi, sizeof formae_nasi / sizeof *formae_nasi, sexus_littera, pulchritudo);
    const char *labra = elige_lineamentum(formae_labrorum, sizeof formae_labrorum / sizeof *formae_labrorum, sexus_littera, pulchritudo);
    ...
    if (aetas <= AETAS_ADULESCENS) indicium_habitus = alea_ponderata(pondera_habitus_iuvenum, 3);
    else if (aetas >= AETAS_MATURUS) indicium_habitus = alea_ponderata(pondera_habitus_senum, 3);
    else indicium_habitus = alea_ponderata(pondera_habitus_adultorum, 3);
    if (pulchritudo >= 3) indicium_habitus = 1;  /* pulchri semper medii vel athletici */
    if (indicium_habitus == 0) habitus = elige_unum(habitus_tenues, sizeof habitus_tenues / sizeof *habitus_tenues);
    else if (indicium_habitus == 1) habitus = elige_unum(habitus_medii, sizeof habitus_medii / sizeof *habitus_medii);
    else habitus = elige_unum(habitus_graves, sizeof habitus_graves / sizeof *habitus_graves);
    ...
    switch (pulchritudo) {
        case 0: pulchritudo_phrasis = elige_vestem(phrases_pulchr_0, sizeof phrases_pulchr_0 / sizeof *phrases_pulchr_0, sexus_littera); break;
        case 1: pulchritudo_phrasis = elige_vestem(phrases_pulchr_1, sizeof phrases_pulchr_1 / sizeof *phrases_pulchr_1, sexus_littera); break;
        case 2: pulchritudo_phrasis = elige_vestem(phrases_pulchr_2, sizeof phrases_pulchr_2 / sizeof *phrases_pulchr_2, sexus_littera); break;
        case 3: pulchritudo_phrasis = elige_vestem(phrases_pulchr_3, sizeof phrases_pulchr_3 / sizeof *phrases_pulchr_3, sexus_littera); break;
        default:pulchritudo_phrasis = elige_vestem(phrases_pulchr_4, sizeof phrases_pulchr_4 / sizeof *phrases_pulchr_4, sexus_littera); break;
    }
    ...
    } while (0)
    {
        ...
        if (alea_fors(0.18)) ADDE_NOTAM(elige_unum(lentigines, sizeof lentigines / sizeof *lentigines));
        if (alea_fors(0.08)) ADDE_NOTAM(elige_unum(naevi, sizeof naevi / sizeof *naevi));
        if (alea_fors(0.06)) ADDE_NOTAM(elige_unum(cicatrices, sizeof cicatrices / sizeof *cicatrices));
        if (aetas >= AETAS_MATURUS && alea_fors(0.7)) ADDE_NOTAM(elige_unum(rugae_tenues, sizeof rugae_tenues / sizeof *rugae_tenues));
        if (aetas >= AETAS_SENEX) ADDE_NOTAM(elige_unum(rugae_altae, sizeof rugae_altae / sizeof *rugae_altae));
    }
    ...
    if (aetas >= AETAS_IUVENIS) {
        ...
        if (indicium_vestium == 0) vestis = elige_vestem(vestes_formales,  sizeof vestes_formales  / sizeof *vestes_formales,  sexus_littera);
        else if (indicium_vestium == 1) vestis = elige_vestem(vestes_cotidianae, sizeof vestes_cotidianae / sizeof *vestes_cotidianae, sexus_littera);
        else if (indicium_vestium == 2) vestis = elige_vestem(vestes_insolitae, sizeof vestes_insolitae / sizeof *vestes_insolitae, sexus_littera);
        else { vestis = elige_vestem(vestes_nudae, sizeof vestes_nudae / sizeof *vestes_nudae, sexus_littera); nudus_electus = 1; }
    } else {
    ...
    if (aetas_mutata_anni >= 0) aetas_anni = aetas_mutata_anni;
    if (arg_vestitus) {
        nudus_electus = 0;
    ...
    fprintf(exitus, "A front-facing portrait photograph of a single person. The "
    ...
    if (sexus == SEXUS_MAS) {
        if (aetas == AETAS_PUER) sexus_nomen = "boy";
        else if (aetas == AETAS_ADULESCENS) sexus_nomen = "teenage boy";
        else sexus_nomen = "man";
    } else {
        if (aetas == AETAS_PUER) sexus_nomen = "girl";
        else if (aetas == AETAS_ADULESCENS) sexus_nomen = "teenage girl";
        else sexus_nomen = "woman";
    }

    fprintf(exitus, "The subject is a %s %d years old, %s with %s skin and %s %s face. ",
           sexus_nomen, aetas_anni, habitus, cutis, articulus(vultus), vultus);

    fprintf(exitus, "%s has %s eyes, %s %s nose, and %s lips, and is %s. ",
    ...
    if (calvus) {
        ...
        if (i_calv == 0) fprintf(exitus, templ_calvitatis[0], Eius, color_comae);
        else if (i_calv == 1) fprintf(exitus, templ_calvitatis[1], Ille, (sexus == SEXUS_MAS) ? "his" : "her", color_comae);
        else fprintf(exitus, templ_calvitatis[2], Eius, color_comae);
    } else {
        ...
        if (i_com == 0) fprintf(exitus, templ_comae[0], Eius, color_comae, capillatura);
        else if (i_com == 1) fprintf(exitus, templ_comae[1], Ille, color_comae, capillatura);
        else fprintf(exitus, templ_comae[2], Ille, color_comae, capillatura);
    }
    ...
    if (perspicilla) {
        ...
        if (i_per == 2) fprintf(exitus, templ_perspicillorum[2], (sexus == SEXUS_MAS) ? "his" : "her", perspicilla);
        else fprintf(exitus, templ_perspicillorum[i_per], Ille, perspicilla);
    }
    ...
    if (arg_fundus) {
        if      (!strcmp(arg_fundus, "planum"))
            ...
            pone_errorem(error_locus, error_longitudo,
        ...
        switch (i_fnd) {
            case 0: fundus_phrasis = elige_unum(fundus_planum,    sizeof fundus_planum    / sizeof *fundus_planum); break;
            case 1: fundus_phrasis = elige_unum(fundus_candidum,  sizeof fundus_candidum  / sizeof *fundus_candidum); break;
            case 2: fundus_phrasis = elige_unum(fundus_ater,      sizeof fundus_ater      / sizeof *fundus_ater); break;
            case 3: fundus_phrasis = elige_unum(fundus_coloratum, sizeof fundus_coloratum / sizeof *fundus_coloratum); break;
            case 4: fundus_phrasis = elige_unum(fundus_textura,   sizeof fundus_textura   / sizeof *fundus_textura); break;
            case 5: fundus_phrasis = elige_unum(fundus_ambitus,   sizeof fundus_ambitus   / sizeof *fundus_ambitus); break;
            default:fundus_phrasis = elige_unum(fundus_exterior,  sizeof fundus_exterior  / sizeof *fundus_exterior); break;
        }
    }
    fprintf(exitus, "The background is %s; it should be simple and not "
...
char *persona_generare_rudis(uint64_t semen,
    ...
    fputs("This is a pictura: a photographic portrait of a realistic "
